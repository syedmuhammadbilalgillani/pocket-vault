...
const AMOUNT_SIGN: Record<string, string> = { expense: "-", income: "+", refund: "+", transfer: "" }
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
      ...
      <div className="flex items-center justify-center gap-3">
        <Button asChild variant="ghost" size="icon-sm">
          <Link href={`/expenses?month=${prevMonth}`} aria-label="Previous month">
            <ChevronLeft />
          </Link>
        </Button>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card size="sm">
          <CardHeader>
            ...
            <CardTitle className="text-lg">{formatMinor(totals.income + totals.refund)}</CardTitle>
          </CardHeader>
        </Card>
        <Card size="sm">
          <CardHeader>
            <CardDescription>Expenses</CardDescription>
            <CardTitle className="text-lg">{formatMinor(totals.expense)}</CardTitle>
          </CardHeader>
        </Card>
        <Card size="sm">
          <CardHeader>
            <CardDescription>Balance</CardDescription>
            <CardTitle className={`text-lg ${balance < 0 ? "text-destructive" : ""}`}>
              {formatMinor(balance)}
            </CardTitle>
          </CardHeader>
        </Card>
      ...
      <CategoryFilter categories={categories} month={format(current, "yyyy-MM")} categoryId={categoryId} />
              ...
              <Card>
                <CardContent className="flex items-center gap-3 py-3">
                  <Link href={`/expenses/${t.id}`} className="min-w-0 flex-1">
                    <p className="truncate font-medium">{t.merchant || t.description || "Transaction"}</p>
                  ...
                  <span className={`shrink-0 font-medium ${AMOUNT_COLOR[t.type]}`}>
                    {AMOUNT_SIGN[t.type]}
