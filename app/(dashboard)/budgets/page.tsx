...
function progressColor(usagePercent: number, isExceeded: boolean, isOverThreshold: boolean) {
...
export default async function BudgetsPage({
  ...
  const budgetByCategoryId = new Map(allBudgets.filter((b) => b.categoryId).map((b) => [b.categoryId!, b]))
  const totalUsage = totalBudget ? usageByBudgetId.get(totalBudget.id) : undefined
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
      ...
      <div className="flex items-center justify-center gap-3">
        <Button asChild variant="ghost" size="icon-sm">
          <Link href={`/budgets?month=${prevMonth}`} aria-label="Previous month">
            <ChevronLeft />
          </Link>
        </Button>
      ...
      {allBudgets.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-8 text-center">
            <PiggyBank className="size-8 text-muted-foreground" aria-hidden="true" />
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              {totalBudget && (
                <CardDescription>
                  {formatMinor(totalUsage?.spentMinor ?? 0)} of {formatMinor(totalBudget.limitMinor)} spent
                </CardDescription>
              )}
            </div>
            <BudgetFormDialog
        ...
        {totalBudget && totalUsage && (
          <CardContent>
            <Progress
              ...
              indicatorClassName={progressColor(totalUsage.usagePercent, totalUsage.isExceeded, totalUsage.isOverThreshold)}
            />
            <p className="mt-2 text-xs text-muted-foreground">
              {totalUsage.usagePercent}% used
              {totalUsage.isExceeded && " — budget exceeded"}
              {!totalUsage.isExceeded && totalUsage.isOverThreshold && " — approaching limit"}
            </p>
          </CardContent>
        )}
      </Card>
      ...
      <div className="flex flex-col gap-2">
        <h2 className="font-heading text-sm font-semibold text-muted-foreground">Category budgets</h2>
        {categories.map((category) => {
          const budget = budgetByCategoryId.get(category.id)
          const categoryUsage = budget ? usageByBudgetId.get(budget.id) : undefined
                  ...
                  <div>
                    ...
                    {budget && categoryUsage && (
                      <p className="text-xs text-muted-foreground">
                        {formatMinor(categoryUsage.spentMinor)} of {formatMinor(budget.limitMinor)}
                      </p>
                    )}
                  </div>
                  <BudgetFormDialog
